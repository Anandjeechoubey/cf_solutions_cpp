...
void solve()
{
    string s;
    cin >> s;
    int countOne = 0, count1 = 0, min = s.length();
    rep(i, 0, s.length())
    {
        if (s[i] == '1')
            countOne++;
    ...
    rep(i, 0, s.length())
    {
        ...
        //11....00 case
        if (min > i + 1 - count1 + countOne - count1)
            min = i + 1 - count1 + countOne - count1;
        //00....11 case
        if (min > s.length() - i - 1 + count1 - countOne + count1)
            min = s.length() - i - 1 + count1 - countOne + count1;
    }
    if (min > countOne)
        min = countOne;
    if (min > (s.length() - countOne))
        min = s.length() - countOne;
    cout << min << endl;
